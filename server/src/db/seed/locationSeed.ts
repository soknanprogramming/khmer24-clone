...
async function seedLocations() {
  try {
    ...
    const existingDistricts = await db.select().from(districtTable);
    ...
    const existingCommunes = await db.select().from(communeTable);
    if (existingCommunes.length > 0) {
      console.log("-> Communes already seeded, skipping...");
    } else {
    ...
    console.error(
      "-> Error seeding fake locations (speed/locationSeep.ts):",
      error,
    );
    process.exit(1);
  }
