...
async function seedOptions() {
  try {
    console.log(
      "-> Starting hardware options seeding (seed/optionsSeed.ts)...",
    );

    console.log("-> Seeding VGA options...");
    const existingVGAOptions = await db.select().from(vgaTable);
    if (existingVGAOptions.length > 0) {
      console.log("-> VGA options already seeded, skipping...");
    } else {
    ...
    const existingCPUOptions = await db.select().from(cpuTable);
    if (existingCPUOptions.length > 0) {
      console.log("-> CPU options already seeded, skipping...");
    } else {
    ...
    const existingRAMOptions = await db.select().from(ramTable);
    if (existingRAMOptions.length > 0) {
      console.log("-> RAM options already seeded, skipping...");
    } else {
    ...
    const existingStorageOptions = await db.select().from(storageTable);
    if (existingStorageOptions.length > 0) {
      console.log("-> Storage options already seeded, skipping...");
    } else {
    ...
    const existingScreenOptions = await db.select().from(screenTable);
    if (existingScreenOptions.length > 0) {
      console.log("-> Screen options already seeded, skipping...");
    } else {
    ...
    console.error(
      "-> Error seeding hardware options (seed/optionsSeed.ts):",
      error,
    );
    process.exit(1);
  }
